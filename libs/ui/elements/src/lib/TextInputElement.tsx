import { Elements } from "@t4g/types";
import { ErrorExclamationIconElement } from "@t4g/ui/icons";
import React, { useEffect, useState } from "react";

export const TextInputElement: React.FC<Elements.TextInputElement.Props> = ({
  testid,
  name,
  label,
  placeholder,
  initialValue,
  description,
  type,
  min,
  max,
  onChange,
  onBlur,
  errorMessage,
  disabled,
  value,
  visible = true,
  className,
  descriptionPosition = "rightOfLabel",
  showError: showErrorProp,
}: Elements.TextInputElement.Props) => {
  const [showError, setShowError] = useState(showErrorProp ?? false);

  useEffect(() => {
    if (errorMessage || showErrorProp) {
      setShowError(true);
    } else {
      setShowError(false);
    }
  }, [errorMessage, showErrorProp]);

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setShowError(false);
    onChange?.(e);
  };

  const handleBlur = (e: any) => {
    onBlur?.(e);
  };
  if (!visible) return null;
  return (
    <div
      className={`TextInputElement w-full ${className || ""}`}
      data-testid="TextInputElement"
    >
      <div className="flex items-center">
        <label
          htmlFor={name}
          className="TextInputElement__label block text-blue-005"
        >
          {label}
        </label>
        {description && descriptionPosition === "rightOfLabel" && (
          <div className="tw-text text-base ml-2.5">{description}</div>
        )}
      </div>
      {description && descriptionPosition === "bottomOfLabel" && (
        <div className="tw-text text-base mb-2.5 mt-1">{description}</div>
      )}
      <div className="relative mt-2">
        <input
          min={min}
          max={max}
          name={name}
          id={name}
          data-testid={testid || ""}
          placeholder={placeholder}
          value={value}
          defaultValue={initialValue}
          type={type || "text"}
          autoComplete={name}
          disabled={disabled}
          className={`form-control `}
          aria-describedby="email-error"
          onChange={handleChange}
          onBlur={handleBlur}
        />

        {showError && (
          <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
            <ErrorExclamationIconElement size="small" />
          </div>
        )}
      </div>
      {showError && (
        <div
          data-testid="TextInputElement__error"
          className="TextInputElement__error text-base text-lm-red dark:text-dm-red pt-2"
        >
          {errorMessage}
        </div>
      )}
    </div>
  );
};
