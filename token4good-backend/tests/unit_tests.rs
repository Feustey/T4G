...
fn test_rgb_service_creation() {
    ...
    assert!(service.is_ok(), "RGB service should be created successfully");
}
...
async fn test_proof_validation() {
    ...
    let result = service.create_proof_contract(
        "mentor_test",
        "mentee_test",
        "request_123",
        10, // Invalid rating
...
async fn test_proof_creation_with_valid_data() {
    ...
    assert!(result.is_ok(), "Proof creation should succeed with valid data");
...
async fn test_rating_boundaries() {
    ...
    // Test rating 0 (valid)
    let result_0 = service.create_proof_contract("m1", "m2", "r1", 0, None).await;
    assert!(result_0.is_ok(), "Rating 0 should be valid");

    // Test rating 5 (valid)
    let result_5 = service.create_proof_contract("m3", "m4", "r2", 5, None).await;
    assert!(result_5.is_ok(), "Rating 5 should be valid");

    // Test rating 6 (invalid)
    let result_6 = service.create_proof_contract("m5", "m6", "r3", 6, None).await;
