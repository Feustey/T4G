...
export default function Custom404({ lang }: ICustom404) {
  const router = useRouter();
  const locale = router.locale as LocaleType;
  return (
    <>
      <Head>
        <title>{lang.page.error.head.error404.title}</title>
        {useIndexing(false)}
      </Head>
        ...
        <section className={`o-hero--error`}>
          ...
          <div>
            <h1 className="u-margin-b--l">
              {lang.page.error.main.title.part1} 404{' '}
            ...
            <p className="u-margin-b--l">{lang.page.error.main.p.error404}</p>
