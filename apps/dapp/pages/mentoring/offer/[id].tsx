...
const Page: React.FC<IPage> & AuthPageType = ({ lang }: IPage) => {
  ...
  const { data: offer, error: offerError, isLoading: isLoadingOffer } = useSwr<MentoringOffer>(
    offerId ? ['mentoring-offer', offerId] : null,
    () => apiClient.getMentoringOffer(offerId),
    { revalidateOnFocus: false }
  );

  const { data: topics = [], isLoading: isLoadingTopics } = useSwr<LearningTopic[]>(
    offerId ? '/api/learning/topics' : null,
  ...
  useEffect(() => {
    if (offer) {
      ...
      setForm({
        topic_slug: offer.topic_slug,
        topic_name: offer.topic?.name ?? offer.topic_slug,
        target_level: (offer.target_level as FormState['target_level']) ?? 'beginner',
        description: offer.description ?? '',
        duration_minutes: offer.duration_minutes ?? 60,
        format: (offer.format as FormState['format']) ?? 'video',
        token_cost: offer.token_cost ?? 60,
        on_demand: avail.length === 0,
        slots: avail.length > 0 ? avail : defaultSlots(offer.duration_minutes ?? 60),
      });
    }
  }, [offer]);
  ...
  async function handleSubmit() {
    ...
    try {
      const payload: Partial<CreateMentoringOfferRequest> = {
      ...
      await apiClient.updateMentoringOffer(offerId, payload);
      router.push('/mentoring/my-sessions?role=mentor&updated=1');
    } catch (e: unknown) {
  ...
  if (!offerId || isLoadingOffer) {
    return (
      <>
        <Head><title>Modifier l&apos;offre — Token4Good</title>{noIndexMeta}</Head>
  ...
  if (offerError || !offer) {
    return (
      <>
        <Head><title>Offre introuvable — Token4Good</title>{noIndexMeta}</Head>
        <ConnectedLayout user={user} lang={lang}>
          <p style={{ color: 'var(--color-error, #d32f2f)' }}>Offre introuvable ou accès refusé.</p>
          <Button variant="secondary" label="Retour aux sessions" onClick={() => router.push('/mentoring/my-sessions')} />
        </ConnectedLayout>
      </>
    );
  }
  ...
  if (offer.status !== 'open') {
    return (
      <>
        <Head><title>Offre non modifiable — Token4Good</title>{noIndexMeta}</Head>
        <ConnectedLayout user={user} lang={lang}>
          <p>Cette offre ne peut plus être modifiée (statut : {offer.status}).</p>
          <Button variant="secondary" label="Retour aux sessions" onClick={() => router.push('/mentoring/my-sessions')} />
        </ConnectedLayout>
      </>
    );
  ...
  if (offer.mentor_id !== user?.id) {
    return (
      <>
        <Head><title>Accès refusé — Token4Good</title>{noIndexMeta}</Head>
        <ConnectedLayout user={user} lang={lang}>
          <p style={{ color: 'var(--color-error, #d32f2f)' }}>Vous ne pouvez modifier que vos propres offres.</p>
          <Button variant="secondary" label="Retour" onClick={() => router.back()} />
        </ConnectedLayout>
      </>
    );
  }
  ...
  if (isSubmitting) {
    return (
      <>
        <Head><title>Modifier l&apos;offre — Token4Good</title>{noIndexMeta}</Head>
        <Spinner lang={lang} spinnerText="Enregistrement…" size="lg" />
      </>
    );
  }
  ...
  return (
    <>
      <Head>
        <title>Modifier l&apos;offre — Token4Good</title>
        {noIndexMeta}
      </Head>
      ...
      <ConnectedLayout user={user} lang={lang}>
        <Breadcrumb
          links={[
            ...
            { text: 'Mes sessions', link: '/mentoring/my-sessions', parent: true },
            { text: 'Modifier l\'offre' },
          ]}
        />

        <h1 className="heading-2">Modifier mon offre de mentoring</h1>
          ...
          {step === 1 && (
            <>
              ...
              {isLoadingTopics ? (
                ...
                <div className="u-d--flex u-flex-column u-gap--m">
                  {Object.entries(topicsByCategory).map(([catName, catTopics]) => (
                    <div key={catName}>
                      <p style={{ fontWeight: 600, fontSize: '0.875rem', margin: '0 0 8px', color: 'var(--color-text-secondary, #64748b)' }}>{catName}</p>
                      <div className="u-d--flex u-flex-column u-gap--xs">
                        {catTopics.map((t) => (
                          <button
                            ...
                            style={{
                              padding: '10px 14px', borderRadius: '0.5rem',
              ...
              {form.topic_slug && (
                <>
                  <div>
                    <label style={{ display: 'block', fontWeight: 600, fontSize: '0.875rem', marginBottom: 6 }}>Niveau ciblé</label>
                    <div className="u-d--flex u-flex-column u-gap--xs">
                  ...
                  <div>
                    <label style={{ display: 'block', fontWeight: 600, fontSize: '0.875rem', marginBottom: 6 }}>Description (optionnel)</label>
                    <textarea
                      rows={3}
                      placeholder="Décris ce que tu vas apporter…"
            ...
            <>
              ...
              <div>
                <label style={{ display: 'block', fontWeight: 600, fontSize: '0.875rem', marginBottom: 6 }}>
                  ...
                  <span style={{ fontWeight: 400, color: 'var(--color-text-secondary)', marginLeft: 8, fontSize: 12 }}>(1 T4G ≈ 15 min)</span>
                </label>
                <input
              ...
              <div>
                <label style={{ display: 'block', fontWeight: 600, fontSize: '0.875rem', marginBottom: 8 }}>Disponibilités</label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14 }}>
                  <input type="checkbox" checked={form.on_demand} onChange={(e) => set('on_demand', e.target.checked)} style={{ cursor: 'pointer' }} />
                  Sur demande uniquement
                </label>
                {!form.on_demand && (
                  <div className="u-d--flex u-flex-column u-gap--xs" style={{ marginTop: 8 }}>
                    {form.slots.map((slot, i) => (
                      <div key={i} className="u-d--flex u-align-items-center u-gap--s">
                        <input
                          ...
                          style={{ flex: 1, padding: '6px 10px', borderRadius: '0.375rem', border: '1.5px solid var(--color-border, #e2e8f0)', fontSize: 13 }}
                        />
                        <button
                          type="button"
                    ...
                    <Button variant="ghost" label="+ Ajouter un créneau" onClick={() => set('slots', [...form.slots, { date: new Date().toISOString(), duration_minutes: form.duration_minutes }])} />
                  </div>
                )}
              </div>
            </>
          ...
          {step === 3 && (
            <>
              ...
              <div className="u-d--flex u-flex-column u-gap--s" style={{ background: 'var(--color-surface-secondary, #f8fafc)', borderRadius: '0.5rem', padding: '1rem' }}>
                ...
                <Row label="Niveau" value={{ beginner: 'Débutant', intermediate: 'Intermédiaire', advanced: 'Avancé' }[form.target_level]} />
                ...
                <Row label="Disponibilités" value={form.on_demand ? 'Sur demande' : `${form.slots.length} créneau(x)`} />
              </div>
              {error && <p style={{ color: 'var(--color-error, #d32f2f)', margin: 0, fontSize: 14 }}>{error}</p>}
            </>
          )}

          <div className="u-d--flex u-gap--s" style={{ marginTop: 8 }}>
            {step > 1 && <Button variant="secondary" label="Retour" onClick={() => setStep((s) => (s - 1) as Step)} />}
            {step < 3 ? (
              <Button
                variant="primary"
                className="flex-1"
              ...
              <Button variant="primary" className="flex-1" label="Enregistrer les modifications" onClick={handleSubmit} />
            )}
          </div>
        </div>
      </ConnectedLayout>
