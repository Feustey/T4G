...
export default function T4gCallback() {
  ...
  useEffect(() => {
    ...
    const processCallback = async () => {
      ...
      try {
        ...
        if (oauthError) {
          console.error('Erreur OAuth t4g:', oauthError);
          setError(`Erreur d'authentification: ${oauthError}`);
          setTimeout(() => {
            router.push('/login?error=t4g_auth_failed');
          }, 2000);
          return;
        }
        ...
        await handleOAuthCallback('t4g', code as string, state as string);
        
      } catch (err) {
        console.error('Erreur callback t4g:', err);
        setError(err instanceof Error ? err.message : 'Erreur inconnue');
        // Réinitialiser en cas d'erreur pour permettre un nouveau essai
        hasProcessedRef.current = false;
        setTimeout(() => {
          router.push('/login?error=t4g_callback_failed');
        }, 2000);
      }
    };
  ...
  if (!mounted) {
    return (
      <>
        <Head>
          <title>Connexion Token4Good...</title>
        </Head>
        <div style={{ 
          display: 'flex', 
  ...
  return (
    <>
      <Head>
        <title>{error ? 'Erreur de connexion' : 'Connexion Token4Good...'}</title>
        <style>{`
          @keyframes spin {
